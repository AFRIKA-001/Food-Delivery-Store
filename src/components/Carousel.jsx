...
const Carousel = ({ slides, autoSlide = true, autoSlideInterval = 4000 }) => {
  ...
  return (
    <div className="relative max-w-6xl  mx-auto overflow-hidden rounded-2xl shadow-xl group group-hover:scale-105 cursor-pointer">
      {/* Slides Container */}
      <div 
      ...
      >
        {slides.map((img, index) => (
          <img 
            ...
            src={img} 
            alt={`Slide ${index + 1}`} 
            className="object-cover w-full h-64 md:h-96 shrink-0"
          />
        ))}
      </div>
